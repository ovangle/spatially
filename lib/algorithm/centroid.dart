...
Coordinate centroidLine(Geometry geom) {
  ...
  void addCoordinates(Iterable<Coordinate> coords) {
    double mid(d1, d2) => (d1 + d2) / 2;
    Iterable<LineSegment> segments = coordinateSegments(coords);
    for (var lseg in coordinateSegments(coords)) {
      centroidSum.x += lseg.midpoint.x * lseg.magnitude;
      centroidSum.y += lseg.midpoint.y * lseg.magnitude;

      totalLength += lseg.magnitude;
    }
  }
...
Coordinate centroidArea(Geometry geom) {
  ...
  void addAllTriangles(Coordinate triangleBase, Iterable<Coordinate> ring, bool isPositiveArea) {
    coordinateSegments(ring)
        .forEach((lseg) => addTriangle(triangleBase, lseg.start, lseg.end, isPositiveArea));
  }
  //A shell contributes a positive area if its coordinates are clockwise
  bool shellPositiveArea(List<Coordinate> coords) =>
      !cg_algorithms.isCounterClockwise(coords);
  //A hole contributes a positive area if its coordinates are counter-clockwise
  bool holePositiveArea(List<Coordinate> coords) =>
      cg_algorithms.isCounterClockwise(coords);

  //Adds the shell
  void addShell(List<Coordinate> shell) =>
  ...
  void addHole(List<Coordinate> shell, List<Coordinate> hole) =>
